...
class TimeSeriesAugmentation(nn.Module):
    ...
    def forward(self, t, x):
        ...
        # 새로운 t와 x 분리
        new_x, new_t = output[ :, :, :self.dim-self.embed_time], output[:, :, self.dim-self.embed_time:]
        return new_x, new_t
...
class enc_mtan_rnn(nn.Module):
    def __init__(self, input_dim, query, num_outputs, latent_dim=2, nhidden=16, 
        ...
        self.aug = TimeSeriesAugmentation(input_dim = input_dim*2+embed_time, hidden_dim = 256, output_dim = input_dim*2+embed_time, embed_time=embed_time, num_outputs=num_outputs)
    ...
    def forward(self, x, t):
        ...
        x_aug, time_steps = self.aug(key, x)
        # print(f"x_aug : {x_aug.shape}, time : {time_steps.shape}")
                
        # x_aug_copy = x_aug.clone()
        mask = torch.where(
            x_aug[:, :, self.dim:2*self.dim] < 0.5,  # 조건
        ...
        out = self.att(query, time_steps, val, mask)
